...
class ProductTypeController extends BaseController
{
    ...
    public function doadd()
    {
        ...
        {
            $this->success('添加成功！', U('ProductType/index'), 1);
        }
		else
		{
			$this->error('添加失败！请修改后重新添加', U('ProductType/index'), 3);
		}
    }
    ...
    public function doedit()
    {
        ...
        {
            $this->success('修改成功！', U('ProductType/index'), 1);
        }
		else
		{
			$this->error('修改失败！请修改后重新添加', U('ProductType/edit',array('id'=>$_POST["id"])), 3);
		}
    }
    
    public function del()
    {
		if(!empty($_GET["id"])){$id = $_GET["id"];}else{$this->error('删除失败！请重新提交', U('ProductType/index'), 3);}if(preg_match('/[0-9]*/',$id)){}else{exit;}
		
		$ProductType = M('ProductType');
		
		if($ProductType->where("reid=$id")->find())
		{
			$this->error('删除失败！请先删除子分类', U('ProductType/index'), 3);
		}
		else
		{
			...
			if($ProductType->where("id=$id")->delete())
			{
				...
				if($Product->where("typeid=$id")->count()>0) //判断该分类下是否有商品，如果有把该分类下的商品也一起删除
				{
					if($Product->where("typeid=$id")->delete())
					{
						...
						$this->success('删除成功', U('ProductType/index'), 1);
					}
					else
					{
						$ProductType->rollback(); // 事务回滚
						$this->error('分类下的商品删除失败！', U('ProductType/index'), 3);
					}
				}
				else
				{
			...
			{
				$ProductType->rollback(); // 事务回滚
				$this->error('删除失败！请重新提交', U('ProductType/index'), 3);
			}
		}
    }
}
