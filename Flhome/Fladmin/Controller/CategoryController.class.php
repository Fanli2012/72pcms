...
class CategoryController extends BaseController
{
    ...
    public function doadd()
    {
        ...
        {
            $this->success('添加成功！', '/'.FLADMIN.'/Category' , 1);
        }
		else
		{
			$this->error('添加失败！请修改后重新添加', '/'.FLADMIN.'/Category' , 3);
		}
    }
    ...
    public function doedit()
    {
        ...
        {
            $this->success('修改成功！', '/'.FLADMIN.'/Category' , 1);
        }
		else
		{
			$this->error('修改失败！请修改后重新添加', '/'.FLADMIN.'/Category/edit?id='.$_POST["id"] , 3);
		}
    }
    
    public function del()
    {
		if(!empty($_GET["id"])){$id = $_GET["id"];}else{$this->error('删除失败！请重新提交','/'.FLADMIN.'/Category' , 3);}if(preg_match('/[0-9]*/',$id)){}else{exit;}
		
		$Arctype = M('Arctype');
		
		if($Arctype->where("reid=$id")->find())
		{
			$this->error('删除失败！请先删除子栏目', '/'.FLADMIN.'/Category', 3);
		}
		else
		{
			...
			if($Arctype->where("id=$id")->delete())
			{
				...
				if($Article->where("typeid=$id")->count()>0) //判断该分类下是否有文章，如果有把该分类下的文章也一起删除
				{
					...
					{
						$Arctype->commit(); // 提交事务
						$this->success('删除成功', '/'.FLADMIN.'/Category' , 1);
					}
					else
					{
						$Arctype->rollback(); // 事务回滚
						$this->error('栏目下的文章删除失败！', '/'.FLADMIN.'/Category', 3);
					}
				}
				else
				{
			...
			{
				$Arctype->rollback(); // 事务回滚
				$this->error('删除失败！请重新提交', '/'.FLADMIN.'/Category', 3);
			}
		}
    }
}
