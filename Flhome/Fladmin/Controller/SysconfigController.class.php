...
class SysconfigController extends BaseController
{
    ...
    public function doadd()
    {
		...
		if($data['varname']!="" && $Sysconfig->data($data)->add())
        {
            ...
            $this->success('添加成功！', '/'.FLADMIN.'/Sysconfig' , 1);
        }
		else
		{
			$this->error('添加失败！请修改后重新添加', '/'.FLADMIN.'/Sysconfig/add' , 3);
		}
    }
    ...
    public function doedit()
    {
        ...
        {
            updateconfig(M("Sysconfig")->select());
            $this->success('更新成功！', '/'.FLADMIN.'/Sysconfig' , 1);
        }
		else
		{
			$this->error('更新失败！请修改后重新提交', '/'.FLADMIN.'/Sysconfig/edit?id='.$_POST["id"] , 3);
		}
    }
    
    public function del()
    {
		if(!empty($_GET["id"])){$id = $_GET["id"];}else{$this->error('删除失败！请重新提交','/'.FLADMIN.'/Sysconfig' , 3);}if(preg_match('/[0-9]*/',$id)){}else{exit;}
		
		if(M("Sysconfig")->where("id=$id")->delete())
        {
            $this->success('删除成功', '/'.FLADMIN.'/Sysconfig' , 1);
        }
		else
		{
			$this->error('删除失败！请重新提交', '/'.FLADMIN.'/Sysconfig', 3);
		}
    }
}