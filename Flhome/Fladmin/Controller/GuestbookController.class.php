...
class GuestbookController extends BaseController
{
    ...
    public function del()
    {
		if(!empty($_GET["id"])){$id = $_GET["id"];}else{$this->error('删除失败！请重新提交', U('Guestbook/index'), 3);}
		
		if(M("guestbook")->where("id in ($id)")->delete())
        {
            $this->success("$id ,删除成功", U('Guestbook/index'), 1);
        }
		else
		{
			$this->error("$id ,删除失败！请重新提交", U('Guestbook/index'), 3);
		}
    }
}
